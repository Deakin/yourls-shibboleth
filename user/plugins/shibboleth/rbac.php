...
function amp_user_has_role( $username, $rolename ) {
	...
	return preg_match($amp_role_assignment[$rolename], $_SERVER[SHIBBOLETH_ENTITLEMENT]);
...
function amp_env_check() {
	...
	foreach ( $amp_role_assignment as $key => $value ) {
		...
		// Christopher and James was here 20210727
//		$t_value = array_map('strtolower', array($value));
		$t_value = strtolower( $value );
...
yourls_add_action( 'activated_shibboleth/plugin.php', 'amp_activated' );
